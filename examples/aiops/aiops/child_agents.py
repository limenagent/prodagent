...
def remediator_agent(*, llm: LLMClient | None = None) -> Agent:
    ...
    return Agent(
        ...
        system_prompt=(
            ...
            "根因 + 回滚目标，然后 rollback（HIGH）。\n"
            "  - OOM / CrashLoopBackOff 且与部署无关: update_incident 写"
            "计划，然后 restart_pod（HIGH）。\n"
            "  - 根因不清: open_incident 然后 page_oncall。\n\n"
            "修复后的验证只有一轮，且必须在同一个 turn 内完成: "
            "get_pod_status + check_slo 各调用一次，然后立刻分叉。"
            "本环境没有「等待」——重复检查只会读到相同的数据并烧掉 "
            "turn 预算，永远不要为了「看是否稳定」而再次检查:\n"
            "  - 已恢复（ready=desired、无 OOMKilled/CrashLoopBackOff/"
            "Pending、SLO 不在 BURNING）: update_incident 设 'resolved' "
            "并写 postmortem，结束。\n"
            "  - 未恢复（有任何坏状态、ready 不足、或读数与之前相同）: "
            "修复未生效。同一个 turn 内 update_incident 记录证据（pod "
            "阶段、pod hash、SLO 数字，status 保持 'mitigated'），然后 "
            "page_oncall，立即结束。不要第三次检查，不要等待 rollout。\n\n"

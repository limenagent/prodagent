...
三个舞台原语共享同一个 `StageDriver`，但共享的边界画得极有分寸。**统一的是循环周围的一切**：生命周期守卫（任何崩溃都收敛为一个终态事件，而不是断流）、分发解释器（串行 / 并发 / 抢答三种调度语义只实现一次）、终止策略（业务策略与硬上限的合取）、预算信封、崩溃后恢复还是重来的判断。**不统一的是循环体本身**：ensemble 挑下一个发言人，blackboard 匹配触发器，queue 扫描过期租约——把这三者强行同构，等于宣称「挑选发言人和回收租约是同一件事」；抽象在这里不是提炼，是抹平。

这给出一条可以带走的判据：**抽走所有人以相同方式需要的东西，留下每个人以不同方式需要的东西。** 前者叫基础设施，后者叫语义。一个框架的品味，最终就体现在这条线画在哪里——画得太靠内，机制不足，每个原语都在重复造轮子；画得太靠外，语义被没收，每个原语都长得像别的原语。
...
### 三种舞台，一套台基：被刻意下沉的公共机械
看完五种原语，你可能会问：Ensemble、Blackboard、WorkQueue 形态差这么多，为什么代码却这么整齐？答案藏在 `coordination/infra/`——**凡是三者一样的东西，只写一遍；凡是三者真的不同的东西，坚决留在各自手里。** 这是整份代码库里"模板方法（Template Method）"思想用得最漂亮的地方，值得单独讲透。

**先看什么是"一样的"。** 不管哪种舞台，跑起来都是同一套生命周期骨架：

```mermaid
graph TD
    A["进入一轮"] --> B["生命周期守卫<br/>抛错→终态 error；无理由结束→unknown"]
    B --> C["按 Activation 派发<br/>serial/concurrent/single_winner"]
    C --> D["预算信封包住每个工作单元<br/>reserve → 干活 → commit"]
    D --> E["终止策略判定<br/>业务策略 ∧ 硬性 MaxRounds"]
    E -->|没停| A
    E -->|停| F["恰好发出一个 *Completed 终态事件"]
```

这套骨架由 `StageDriver` 一次性实现：生命周期守卫、派发解释器、终止判定、预算信封、"恢复还是新开"的耐久判断。三个舞台各自提供**不同的那一小块**——"一轮具体干什么"（Ensemble 选发言者、Blackboard 匹配触发器、WorkQueue 扫租约并扇出工人）和"为什么停"（Ensemble 报预算、Blackboard 报无贡献、WorkQueue 报清空/无进展）。

> **小白加餐：模板方法 vs 策略模式。** 模板方法指的是：父类把"算法的固定步骤顺序"写死（这里就是上面那张流程图），把会变化的步骤留成"填空"交给子类。策略模式指的是：把"某一步怎么做"抽成一个可替换对象（这里的 `ActivationPolicy` 决定谁上、`TerminationStrategy` 决定何时停）。prodagent 两者结合：**骨架用模板方法固定，变化点用策略对象插拔。**

这里有一个非常成熟的克制：模块注释里特意写道——*轮体和停止原因故意留在子类里，强行统一会抹掉真实的语义差异。* 抽象不是越彻底越好，**把"形似而神不同"的东西硬合并，只会得到一个充满 if-类型 分支的伪复用。** 只在真正同构的地方下沉，是这层设计的分寸感。

**共享的状态底座也分两层。** 三个舞台都要驱动一块"共享可变状态"，于是有了 `SharedStore` 这个最窄的读侧契约（都要能数轮次 `round_count`、打快照 `snapshot`、算活性指纹 `fingerprint`）：

| 底座 | 谁用 | 为什么 |
|------|------|--------|
| `SharedStore` | `SharedFloor`（Ensemble 的发言记录） | 单写者、追加式，天然不需要锁，轮次直接从发言数推出来 |
| `RoundedLockableStore` | `Board` / `SharedQueue` | 每轮要并发扇出多个专家/工人，需要一把锁 + 显式轮次计数 |
| `EventSourcedStore`（混入） | 需要崩溃恢复的舞台 | 每次状态变更往 EventLog 追加一条事件，复用乐观尾序号检查 |

> **小白加餐：mixin（混入）是什么？** 它是一种"不通过继承主干、只横向叠加一小撮能力"的手法。`EventSourcedStore` 不关心你是 Floor 还是 Queue，只提供"把一次状态变更追加进事件日志"这一个能力，谁需要耐久谁就混入它。这比把"持久化"塞进所有共享状态的基类要灵活得多。

**最后是收场：`Settler`。** 一条 run 链（可能跨了好几个 peer）无论中间经历了什么，在**结束的那一刻要做的事是固定的**：结构化输出校验、把最终结果作为一条 UPSTREAM 消息送回调用方、落 checkpoint、跑 RUN_COMPLETE 门禁、发终态事件。这些"只发生一次、且必须发生一次"的动作被收进 `Settler`，于是各个驱动器不必各自记得收尾，也不会出现"某条路径忘了落盘"的漏洞。

**一句话总结这层设计**：`StageDriver` 固定流程骨架，`SharedStore` 家族固定状态底座，`Settler` 固定收场纪律，而"谁上、怎么停、一轮干什么"这些真正不同的部分以策略和子类的形式留下。你要新增第四种协作玩法（比如一个 LLM 当主持人的舞台），要写的只是"填空"，而不是再复制一遍轮循环、预算、收尾——**好的抽象让新增一种玩法变成填填空题，而不是抄一遍问答题。**
...
- 想看真实的多 Agent 示例？→ [10 个端到端示例 →](../examples.md)

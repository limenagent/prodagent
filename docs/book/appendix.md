...
全书的基础知识都散在各章的"插曲"小节里，按需教的。这里把它们集中成一张索引：忘了某个概念，查到这里，回到那一章复习。每条给一句话提示和出处。
...
| StrEnum | 封闭的值集合 + 值即字符串可直落 JSON | [1.5](ch01.md) |
| `if TYPE_CHECKING` | 只在类型检查时生效的 import，运行时零开销 | [2.2](ch02.md) |
| 裸星号参数 | 之后的参数强制关键字传递，调用处自解释 | [2.2](ch02.md) |
| 惰性导入 | 目录常开、按需取书；"轻"由白名单测试看守 | [1.6](ch01.md) |
...
| StageDriver | 三种舞台共用的驱动器：共享循环外围，不共享循环体 |
...
| quiz_arena | 多 Agent 辩论（ensemble） | 10 |
| code_detective | 黑板协作（blackboard） | 10 |
| aiops | 工作队列运维（work_queue） | 10 |
| council | 模型自己召集协作 | 10 |

...
现在可以读懂循环内核的主入口了（`src/prodagent/kernel/react.py`，有删节）：
...
所以框架把这些转移收进了 `AgentRun` 的四个方法（`src/prodagent/kernel/state.py`，源码里这段的注释标题就叫 "Terminal transitions — the single throat"，终态转移的单咽喉——"咽喉"是比喻：所有必经之路收窄成唯一一条通道，守住一条就守住了全部）：
...
状态是骨架，干活的是每一轮。框架把"一轮"封装成一个类，叫 `Turn`（`src/prodagent/kernel/turn.py`）。先看它一轮要过的全部关卡——这不是示意图，每一步都对应真实代码路径：
...
`_think` 内部调用 `_call_llm`，超时就是在这层实现的，把这一段读完整（`src/prodagent/kernel/turn.py`，有删节）：
...
| 内容 | 源码位置 |
|------|---------|
| ReactEngine（循环策略） | `kernel/react.py` |
...
然后打开 `src/prodagent/kernel/react.py`，找到 `stream()`，对照着看：它的形状和你刚写的是同一个形状，只是产出的事件更丰富。

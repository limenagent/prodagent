...
Step 的 `run()` 方法是一轮的完整原子，docstring 写得非常清楚：先想（组装上下文、调用模型、记账），只有当模型要求工具时才做（执行工具批次），且预算在批次前后各查一次——因为模型调用本身可能就把上限烧穿了。真码：
...
`_think` 内部调用 `_call_llm`，超时就是在这层实现的，把这一段读完整（`src/prodagent/kernel/step.py`，有删节）：
...
裸循环的第一个漏洞（模型打转转到底）在 Step 的入口处被拦。原理不复杂：给每一轮的工具调用算一个**指纹**——一段简短的标识，内容相同的调用指纹相同。指纹怎么算？用**哈希**：把任意数据映射成一小段固定长度摘要的算法，相同输入必得相同摘要，好比给任意大的文件算出一个"身份证号"。框架保留最近若干轮的指纹做比对，连续多轮指纹一模一样，判定为死循环，抛 `InfiniteLoopDetected` 停机。这比单纯限最大轮数聪明：上限 20 轮的 run 可能第 5 轮就进了死循环，指纹检测第 7 轮就能叫停，省下白烧的十几轮。
...
答案是把"循环"拆成两个职责。`ReactiveLoop` 负责**循环策略**：什么时候停、怎么恢复、异常怎么结算（还记得 3.2 的 `_settle_terminated` 吗，结算的活归它）、终止事件怎么发。`Step` 负责**一轮的原子执行**：想、做、记账。两者一小一大：

...
现在可以读懂循环内核的主入口了（`src/prodagent/runtime/agent_loop.py`，有删节）：
...
所以框架把这些转移收进了 `AgentRun` 的四个方法（`src/prodagent/kernel/run.py`，源码里这段的注释标题就叫 "Terminal transitions — the single throat"，终态转移的单咽喉——"咽喉"是比喻：所有必经之路收窄成唯一一条通道，守住一条就守住了全部）：
...
状态是骨架，干活的是每一轮。框架把"一轮"封装成一个类，叫 `Round`（`src/prodagent/runtime/agent_loop.py`）。先看它一轮要过的全部关卡——这不是示意图，每一步都对应真实代码路径：

```mermaid
graph TD
    START["进入 Turn"] --> CHECK1["① 预算检查<br/>四轴"]
...
Turn 的 `run()` 方法是一轮的完整原子，docstring 写得非常清楚：先想（组装上下文、调用模型、记账），只有当模型要求工具时才做（执行工具批次），且预算在批次前后各查一次——因为模型调用本身可能就把上限烧穿了。真码：
...
签名里的 `AsyncIterator[AgentEvent]` 和 3.2 的 `AsyncGenerator` 是近亲：前者是"能被 `async for` 消费的东西"的统称，后者特指"用 `yield` 产出的那种"；实现者视角写 Generator，消费者视角写 Iterator，这里 Turn 是被循环消费的一方，所以用 Iterator。一轮从 `_think` 开始：组装发给模型的上下文（记忆召回、压缩、技能注入都在这一步，第 6 章讲），调用模型，记账。然后看模型的 `stop_reason`：说完了就结束本轮（`_end_turn`），要工具就先查一次预算，执行工具批次，再查一次预算。注意这个方法本身也是异步生成器——一轮内部发生的事（模型想说的每个字、每个工具的结果）同样是流出去的事件。

### 每个工作单元也有自己的一生：NodeStatus 六态

刚才 3.3 讲的是**整个运行**的状态（RunState，四态）。现在补上**单个工作单元**（一步）的状态，它叫 `NodeStatus`，定义在 `src/prodagent/kernel/types.py`，一共六个值。这是全书唯一一次正式讲它，第 8 章规划器里每个图节点的 `status` 字段用的就是它。读法和刚才的 RunState 状态图完全一样：
...
`_think` 内部调用 `_call_llm`，超时就是在这层实现的，把这一段读完整（`src/prodagent/runtime/agent_loop.py`，有删节）：
...
裸循环的第一个漏洞（模型打转转到底）在 Turn 的入口处被拦。原理不复杂：给每一轮的工具调用算一个**指纹**——一段简短的标识，内容相同的调用指纹相同。指纹怎么算？用**哈希**：把任意数据映射成一小段固定长度摘要的算法，相同输入必得相同摘要，好比给任意大的文件算出一个"身份证号"。框架保留最近若干轮的指纹做比对，连续多轮指纹一模一样，判定为死循环，抛 `InfiniteLoopDetected` 停机。这比单纯限最大轮数聪明：上限 20 轮的 run 可能第 5 轮就进了死循环，指纹检测第 7 轮就能叫停，省下白烧的十几轮。
...
答案是把"循环"拆成两个职责。`ReactEngine` 负责**循环策略**：什么时候停、怎么恢复、异常怎么结算（还记得 3.2 的 `_settle_terminated` 吗，结算的活归它）、终止事件怎么发。`Turn` 负责**一轮的原子执行**：想、做、记账。两者一小一大：
...
PLAN_FIRST 模式的循环策略完全不同——要管理步骤依赖、并行执行、断点续跑（第 8 章细讲）——但它的每一步，仍然是同一个"想→做→记账"的原子。把 Turn 抽出来，两种模式共享同一段被充分测试过的代码；换模式只换策略层。你在第 1 章看到的"共用同一套大脑、手、预算和恢复"，落到代码里就是这个拆分。

这个拆分还有一层更深的含义：**Turn 是可恢复的最小单位**。很多框架把多轮循环写成一个巨大的函数，中间状态散落在局部变量里——进程一死，什么都留不下，根本无从恢复。框架把每一步的状态全部收敛到 `AgentRun` 对象上（第 7 章你会看到它整体序列化落盘），Turn 才成为"恢复时可以精确对齐"的刻度：死在 Turn 中间，恢复逻辑知道该从哪条边续跑。能恢复的前提不是"写了恢复代码"，是"状态有家可归"。
...
| 内容 | 源码位置 |
|------|---------|
| AgentLoop（循环策略） | `runtime/agent_loop.py` |
| Turn（轮原子） | `kernel/turn.py` |
...
然后打开 `src/prodagent/runtime/agent_loop.py`，找到 `stream()`，对照着看：它的形状和你刚写的是同一个形状，只是产出的事件更丰富。
...
练习 3：把 NodeStatus 六态读进代码。打开 `src/prodagent/kernel/types.py` 找到 `NodeStatus`，逐个值读一遍它上面的注释，重点读 OBSOLETE 那段。然后 `grep -rn "NodeStatus.OBSOLETE" src/prodagent/`，看它在第 8 章的规划器里到底在哪里被赋值——先留个印象，第 8 章正式讲。

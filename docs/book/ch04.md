...
四轴任何一个触顶，`BudgetExceeded` 异常被抛出，第 3 章 `stream()` 的 except 分支接住它，进入结算：run 的状态走单咽喉转成 FAILED，异常经过分类落进崩溃现场，追踪 span（span 指一次操作从开始到结束的完整追踪记录，第 11 章的主角）记下错误，checkpoint 保存最终状态供事后分析。错误信息里带着具体数字：哪个轴、当前值多少、上限多少。比如轮数轴的报错是 "Round limit reached: 20/20"。

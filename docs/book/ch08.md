# 第 8 章 · 规划与 DAG
> 阅读时间约 60 分钟。本章讲"先规划后执行"这种模式，开头先补一块图论基础：有向无环图与拓扑序。
>
...
DAG 天然适合表达"依赖"。每个节点是一步工作，一条从 A 指向 B 的边说"B 要等 A 完成"。查资料 A、读文档 B、汇总对比 C、写报告 D，这四步的依赖图画出来是这样：

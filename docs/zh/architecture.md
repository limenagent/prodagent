...
**第二，把步骤之间的关系变成显式的图。** while 循环把“先做什么后做什么”焊死在代码里，想换
一种编排就得重写。框架把步骤（`Node`）和步骤之间的连线（`Edge`）抽成一张数据化的图
（`Plan`），换编排等于换一张图，驱动执行的引擎不动。
...
在这三个动作之上，为了把“能恢复、能并发、能暂停、能看见”做扎实，再补四个部件。整张图如下：
...
flowchart TB
  ...
  subgraph K["内核（机制）"]
    direction TB
    P["Plan：Node 节点 / Edge 连线 / Channel 状态通道"]
    R["Run：一次执行的状态与生命周期"]
    S["Scheduler：算就绪 → 波次并发 → 屏障折叠 → 落检查点"]
    L["EventLog：事件是事实源，State 是折叠出来的投影"]
    B["Bus：把每个动作对外广播；Interrupt：随时能停下等人"]
    P --> R --> S
    S --> L
    S --> B
  end
...
| 部件 | 它解决的问题 |
|---|---|
| **Plan** | 有哪些步骤、怎么连、状态长什么样——一张可复用的静态图纸 |
| **Node / Edge / Channel** | 节点是步骤，边是依赖关系，通道规定并发写入时状态怎么合并 |
...
一台 Agent 引擎听起来很复杂，但剥到底就七个部件，而且它们是被同一个问题一步步推导出来的：
要能恢复，就得有显式状态；要灵活编排，就得有显式的图；要驱动图，就得有调度器；要并发不出错、
要能暂停、要对外可见，才依次长出了通道、事件日志、总线和中断。**不是谁拍脑袋设计了七个概念，
是问题本身把它们要了出来。**

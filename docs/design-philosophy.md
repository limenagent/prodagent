...
> 1,300+ 个测试零 API key、零网络、零 flaky。FakeLLM 精确控制每一轮输出。
...
**2. 测试速度**——1,300+ 个测试在 30 秒内跑完。因为没有网络往返。
...
确定性解决的是「测试可信」，接下来的问题是「正确性可证」。例子测试验证的是**你想过的路径**，而系统里最致命的错误往往藏在没人走过的路径里。对这类风险，prodagent 的做法是把核心不变式写成**定律**，交给属性测试对任意输入做机器检验：

- checkpoint 往返：`from_dict(to_dict(run)) == run`，对任意 run 成立；
- 恢复等价：任意事件序列、任意切分点，前缀快照 + 尾部重放 ≡ 全量重放——恢复的可靠性是被证明的，不是被习惯相信的；
- 账本恒等：任意 reserve/commit/release 序列下，`spent == committed + reserved`。

例子说「我看到它对过」；定律说「它不可能错」。两者的差距，就是抽查和证明的差距——写法与实例见 [测试与评估专题 →](topics/evaluation.md)。

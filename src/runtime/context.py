...
def _render(messages: list[dict]) -> str:
    ...
    for m in messages:
        role = m.get("role", "?")
        content = m.get("content") or m.get("text") or str({k: v for k, v in m.items() if k != "role"})
...
class CompressionLevel:
    ...
    NAME = {0: "NONE", 1: "TOOL_COMPRESS", 2: "HISTORY_SUMMARY",
            3: "TOPIC_SUMMARY", 4: "EMERGENCY"}
...
class TieredCompactionContext:
    ...
    def __init__(self, summarizer: Any, *, capacity: int = 12,
                 history_recent: int = 6, topic_recent: int = 3):
    ...
    async def assemble(self, messages: list[dict]) -> list[dict]:
        ...
        if level == CompressionLevel.TOOL_COMPRESS:
            ...
            shrunk = [{**m, "content": _shrink_tool_text(str(m.get("content", "")))}
                      if m.get("role") == "tool" else m for m in messages]
        ...
        last_summary = next((m for m in reversed(messages)
                             if str(m.get("content", "")).startswith(("[历史摘要]", "[主题摘要]"))), None)

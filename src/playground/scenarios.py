...
def _greeter():
    ...
    return Agent(
        ...
        instruction="你是奶茶店助手，回答简洁。", tools=[menu])
...
def _trader():
    ...
    buyer = Agent(
        name="buyer",
        model=_model(
            ToolCall("quote", {}), ToolCall("quote", {}),
            "两轮砍价后谈到 14 元自取，准备下单。"),
        instruction="你是代购助手，先砍价再申请下单。", tools=[quote],
        memory=InMemoryMemory(), bus=wf.bus)          # 子 Agent 事件汇入同一总线
    ...
    wf.branch("approve", {"place_order": "place_order", "cancel": "cancel"},
...
def _research():
    ...
    return Agent(
        ...
        instruction="你是行业研究员。", tools=[search],
...
def _compliance():
    ...
    async def freeze(summary, ctx):
        ...
        return go("report", summary,
                  decision="已冻结 A1、A2" if ctx.resume_value.get("approved")
                  else "建议冻结，但本次未获批准")
...
async def _detective():
    ...
    return Agent(
        ...
        instruction=system, registry=registry)
...
def _aiops():
    ...
    def engineer(name, *script, tools=None):
        return Agent(name, model=_model(*script),
                     instruction=f"你是{name}，先用工具查数据再下结论，两句话内给出结论。",
                     tools=tools or [], bus=wf.bus)
...
def _review_team():
    ...
    wf.branch("judge", {"revise": "revise", "finalize": "finalize"},
              decide=lambda s: s.get("verdict"))
    wf.entry("writer")
    return wf
...
async def _memory_regular():
    ...
    return Agent(
        name="regular",
        model=_model("好的，按你记忆中的偏好做了无糖去冰，已下单。",
                     "记得呢——还是无糖去冰，这就再帮你下一单。"),
        instruction="你是点单助手，要结合长期记忆里的偏好。", memory=mem)


# 场景表：key -> 标题、简介、默认输入、构建器（可能是 async）。
SCENARIOS = [
    {"key": "01", "title": "01 问候点单", "default": "你们这儿有芋泥啵啵吗？",
     "desc": "最小的 Agent：想一步、调一次工具、再回答。", "build": _greeter, "is_async": False},
    {"key": "02", "title": "02 代购砍价·下单审批", "default": "帮我买杯奶茶，尽量便宜",
     "desc": "多轮砍价后，动钱前在网页上挂起等你批准。", "build": _trader, "is_async": False},
    {"key": "03", "title": "03 深度研究·上下文压缩", "default": "帮我研究新能源赛道",
     "desc": "连查多轮，超预算的早期检索被压成摘要。", "build": _research, "is_async": False},
    {"key": "04", "title": "04 合规审计·并行+审批", "default": "审计账户 A1",
     "desc": "两路并行核查、汇合，冻结前挂起审批。", "build": _compliance, "is_async": False},
    {"key": "05", "title": "05 代码侦探·MCP+技能", "default": "test_x 一直红，帮我修好",
     "desc": "MCP 工具在边界拉平，按技能指引失败再改到转绿。", "build": _detective, "is_async": True},
    {"key": "06", "title": "06 行程规划·并行子 Agent", "default": "上海两日游",
     "desc": "同一波并行派三个专业 Agent，再汇合合成。", "build": _trip, "is_async": False},
    {"key": "07", "title": "07 故障应急·委派与接力", "default": "订单服务延迟飙升",
     "desc": "并行委派诊断（call），再 handoff 给修复 Agent（transfer）。",
     "build": _aiops, "is_async": False},
    {"key": "08", "title": "08 撰稿-审阅-修订·多 Agent", "default": "写一份季度经营结论",
     "desc": "撰稿/审阅/修订三个 Agent，按审阅结果走条件分支再汇合定稿。",
     "build": _review_team, "is_async": False},
    {"key": "09", "title": "09 长期记忆·跨会话召回", "default": "帮我点杯奶茶",
     "desc": "记忆独立于本次对话存在，think 前检索并注入，模型像记得你。",
     "build": _memory_regular, "is_async": True},
]

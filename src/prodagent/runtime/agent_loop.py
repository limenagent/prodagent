...
The kernel sees this machinery only through the ``AutonomyEngine`` port
(kernel/unit.py) — ``AutonomousUnit`` drives whatever implements it. The
...
class AgentLoop:
    """Round-after-Round execution of one run — what an AutonomousUnit runs."""
    ...
    def bind_clock(self, clock: RecordingTimePort | None) -> None:
        """The Scheduler owns the run's frozen-clock recorder; the loop
    ...
    def outcome_of(self, run: Run, *, goal_scope: bool = False) -> ToolResult:
        ...
        AutonomousUnit reports into the node lifecycle (success /
        suspended / handoff)."""
        if run.pending_handoff is not None:

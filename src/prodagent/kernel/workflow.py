"""Workflow — the kernel's hand-written plan builder.

A declaration, not wiring: names nodes, their bodies (fn / tool / llm /
agent), their dependencies, and the plain functions its fn nodes invoke —
nothing else. No LLM client, no hooks, no tool registration: those belong
to the composition root, injected at execution time. That is what makes a
compiled Workflow reusable across runs and harmless to share. (Lives in
the kernel because that is all it is: pure sugar over Plan/Node/Edge; the
@workflow code front-end grows beside it.)

``compile()`` freezes the declaration into the same immutable ``Plan``
any front-end produces; an agent with a preset plan executes it as-is.
...
class _NodeSpec:
    ...
    node_id: str
    body: FnBody | ToolBody | LLMBody | SubPlanBody
...
class Workflow:
    ...
    def fns(self) -> dict[str, Callable[..., Any]]:
        ...
        composition root so the scheduler's NodeContext can resolve them at execution."""
    ...
    def _register_agent_step(
        ...
        *,
        ...
        self._specs.append(
            _NodeSpec(
                node_id=node_id,
                body=SubPlanBody(agent=agent.name, task=f"Execute {node_id}"),
                params=merged,
                depends_on=list(depends_on or []),
                is_terminal=is_terminal,
            )
...
def _compile_workflow(
    ...
    with the offender named — not as a hang at run. (Folded in from the
    old plan/ir front-end: the builder and its gate live together.)"""

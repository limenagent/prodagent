"""NodeBody — the one composable interface: everything runnable is a body.

An Agent, a governed tool call, a fixed-prompt model call, a loop body
(recipes), a child activation, a whole subgraph — all of them are "a
...
and micro agents, only bodies nesting bodies.

Two vocabularies live beside the interface:

- :class:`Outcome` — what a body's run produces: a value, a state delta to
  merge under the run's reducer rules, and an explicit control-flow choice.
- :class:`NodeContext` — the execution wiring a body may draw on: the tool
  throat, the model invoker, the activation slot, the loop engine, the fn
...
context of injected collaborators reconstructs any body; bodies themselves
...
# are the *service slots* a NodeContext may carry. The composition root fills
# them; bodies draw on them through the context. Keeping them as narrow
...
class NodeBody(Protocol):
    ...
    ``run`` is the whole contract: input in, :class:`Outcome` out — a plain
    class with a ``run`` method satisfies this structurally, which is the
    column's first-class principle: a body is a replaceable executor, and a
    plain function-shaped executor is a body. A body with live events (a
    think-act loop's rounds) may also expose the same execution as a
    generator via :class:`StreamingBody` — drivers that stream prefer that
    form; ``ctx.fire`` is the model-agnostic fallback.

    The metadata face — ``kind``, ``readonly``, ``target`` — is what makes
    a body *graph-addressable*: a display target, a kind tag (``NodeKind``
        ...
        if raw.get("suspended"):
            return ToolResult.suspended(
                ...
                approval_request_id=str(
                    raw.get("approval_request_id") or raw.get("request_id") or ""
                ),
                interrupt_kind=str(raw.get("interrupt_kind", "")),
            )
        if raw.get("handoff"):
...
class StreamingBody(Protocol):
    """Optional companion form of :meth:`NodeBody.run` for bodies with live
    events.

    ``run_stream`` is the *same* execution as ``run``, exposed as an async
    generator: live events (a think-act loop's rounds) are yielded as they
    ...
    *inline on their own stack*: suspending the driver suspends the body
    ...
    def run_stream(
        self, input: Any, ctx: NodeContext, box: list[Outcome]
...
@dataclass(slots=True)
class NodeContext:
    """The execution wiring one body may draw on — never run data.

    What a context holds: who is executing (``run_id``/``node_id``), the
    live run it reads from (conversation, metrics — the loop engine writes
    ...
    activation slot, the service bag, the fn table). ``emit`` is where
    ...
    instead of being baked into every body declaration."""
    ...
    wiring: Mapping[str, Any] = field(default_factory=dict)
    """The composition root's service bag: string keys to live
    collaborators a *specific* body asks for by convention (the loop
    recipe's driver, say). The kernel only carries it — it never reads a
    key, so no capability can accrete into the kernel through this slot."""

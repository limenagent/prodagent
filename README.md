...
- **多 Agent 协作**：什么时候该拆多 Agent，什么时候单 Agent 加好上下文管理就够了？三种阶段驱动拓扑（投票 ensemble、共享黑板 blackboard、工作队列 work_queue）和两种委派策略（垂直委派 spawn、水平接力 peer）分别适用于什么场景，怎么选？Agent 间通信怎么保证不丢不重不乱序？多个 Agent 互相甩锅进入死循环，怎么兜底？

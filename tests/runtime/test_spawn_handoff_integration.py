...
async def test_spawn_tool_respects_duplicate_message_ids():
    ...
    spawn_tools = build_spawn_tools_for_agent(
        [child], runner=InProcessRunner(ParentRuntime(llm=fake_llm))
    )
...
async def test_spawn_tool_meta_allows_parallel_and_idempotent():
    ...
    spawn = build_spawn_tools_for_agent(
        [_spec()], runner=InProcessRunner(ParentRuntime(llm=fake_llm))
    )
...
async def test_spawn_aggregates_cost_into_accumulator():
    ...
    fake_llm = script({"content": "done"})
    ctx = ParentRuntime(llm=fake_llm)
    spawn = build_spawn_tools_for_agent(
        [_spec()], runner=InProcessRunner(ctx), accumulator=ctx.accumulator
    )
...
async def test_l7_handoff_rejection_dead_letters(monkeypatch=None):
    ...
    fake_llm = script({"content": "done"})
    spawn = build_spawn_tools_for_agent(
        [_spec()], runner=InProcessRunner(ParentRuntime(llm=fake_llm)), hooks=hooks
    )
    result = await spawn.tool._fn(name="worker", task="do it")
...
async def test_security_veto_from_child_propagates_not_swallowed():
    ...
    spawn = build_spawn_tools_for_agent(
        [_spec()], runner=InProcessRunner(ParentRuntime(llm=_VetoLLM()))
    )

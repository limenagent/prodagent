...
async def test_diamond_parallel_reducers():
    ...
    p.add(Node("a", FnBody(lambda x, ctx: Outcome.ok(None, items=["a"], n=1))),
    ...
    p.edge("a", "b"); p.edge("a", "c"); p.edge("b", "d"); p.edge("c", "d")
    run = await run_plan(p)
    assert run.state == RunState.COMPLETED
...
async def test_conditional_branch_skips_dead_path():
    ...
    p.add(Node("start", FnBody(lambda x, ctx: Outcome.ok(None, go_left=True))),
...
async def test_goto_loop():
    ...
    def tick(x, ctx):
        return (Outcome.goto("tick", c=1) if ctx.shared["c"] < 2
                else Outcome.ok("stop", c=1))
...
async def test_dynamic_fanout_fanin():
    ...
    p.add(Node("fan", FnBody(lambda x, ctx: Outcome.fan_out(
...
async def test_stagnation_fails():
    p = Plan()
    p.add(Node("a", FnBody(lambda x, ctx: Outcome.ok("A"))),
